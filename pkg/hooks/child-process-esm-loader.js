...
export const resolve = async function resolve(specifier, context, nextResolve) {
    ...
    try {
        ...
        try {
            debugLog?.("oxc and node backup resolve error", { resolveError: resolveError.message });
...
export const load = async function load(url, context, nextLoad) {
    ...
    const format = context.format ?? (await getPackageType(fileURLToPath(url))) ?? "commonjs";
    if (format == "commonjs") {
...
async function getPackageType(path, isFilePath) {
    try {
        isFilePath ?? (isFilePath = await fs.readdir(path).then(() => false));
    }
    catch (err) {
        if (err?.code !== "ENOTDIR") {
            throw err;
    ...
    if (dir.length > 1)
        return await getPackageType(resolvePath(dir, ".."), false);
    return undefined;
}
