...
utils_1.log.debug("syncworker file boot", { isMainThread: worker_threads_1.default.isMainThread });
/**
...
class SyncWorker {
    constructor(scriptPath) {
        ...
        this.worker.on("error", (error) => {
            utils_1.log.error("[esbuild-dev] Internal error", error);
            process.exit(1);
        });
        this.worker.on("exit", (code) => {
            if (code !== 0) {
                utils_1.log.error("[esbuild-dev] Internal error, compiler worked exited unexpectedly");
                process.exit(1);
            }
        });
    ...
    call(...args) {
        ...
        if (status === "timed-out")
            throw new Error("[esbuild-dev] Internal error: timed out communicating with esbuild-dev sync worker thread, likely an esbuild-dev bug");
        ...
        if (!message)
            throw new Error("[esbuild-dev] Internal error: no response received from sync worker thread");
        const response = message.message;
        if (response.id != id)
            throw new Error(`[esbuild-dev] Internal error: response received from sync worker thread with incorrect id, sent ${id}, recieved ${response.id}`);
        if (response.error)
...
if (!worker_threads_1.default.isMainThread) {
    const runWorker = async () => {
        ...
        if (!workerData)
            return setImmediate(runWorker); // eslint-disable-line @typescript-eslint/no-implied-eval
        const file = process.env["ESBUILD_DEV_DEBUG"] ? await fs_1.promises.open("/tmp/esbuild-dev-debug-log.txt", "w") : undefined;
