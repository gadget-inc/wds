...
export class Supervisor extends EventEmitter {
    constructor(argv, socketPath, options, project) {
        ...
        // Handle IPC channel errors that can be emitted asynchronously
        // These errors can't be caught by try-catch blocks
        process.on("error", (error) => {
            if (error.code === "ERR_IPC_CHANNEL_CLOSED") {
                log.warn(`WDS error: parent process IPC channel closed: ${error.message}`);
            }
            else {
                // Re-throw other errors as we don't want to swallow them
                throw error;
            }
        });
    }
    ...
    restart() {
        ...
        const onChildProcessMessage = (message) => {
            if (process.send) {
                try {
                    ...
                    if (error.code === "ERR_IPC_CHANNEL_CLOSED") {
                        log.debug(`Cannot forward message to parent: IPC channel closed`);
                    }
                    else {
                        throw error;
                    }
        ...
        const onParentProcessMessage = (message) => {
            // Check if child process is still connected before sending
            if (this.process && this.process.connected) {
                try {
                    ...
                    if (error.code === "ERR_IPC_CHANNEL_CLOSED") {
                        log.debug(`Cannot forward message to child: IPC channel closed`);
                    }
                    else {
                        throw error;
                    }
        ...
        // Handle errors from child process IPC channel
        this.process.on("error", (error) => {
            if (error.code === "ERR_IPC_CHANNEL_CLOSED") {
                log.debug(`Child process IPC channel closed: ${error.message}`);
            }
            else {
