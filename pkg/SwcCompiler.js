...
class SwcCompiler {
    ...
    async getModule(filename) {
        const root = (0, find_root_1.default)(filename);
        const config = await (0, utils_1.projectConfig)(root);
        let swcConfig;
        if (!config.swc || typeof config.swc === "string") {
            swcConfig = {
                swcrc: true,
                configFile: config.swc && config.swc !== ".swcrc" ? path_1.default.resolve(root, config.swc) : undefined,
            };
        }
        ...
        let fileNames = await (0, globby_1.default)(globs, { cwd: root, absolute: true });
    ...
    async buildFile(filename, root, config) {
        const output = await (0, core_1.transformFile)(filename, {
            cwd: root,
            filename: filename,
        ...
        await (0, write_file_atomic_1.default)(destination, output.code);
    ...
    async reportErrors(results) {
        for (const result of await results) {
            if (result.status === "rejected") {
                utils_1.log.error(result.reason);
            }
        }
    }
    ...
    async isFilenameIgnored(filename) {
        ...
        const actual = await (0, globby_1.default)([...includeGlobs, ...ignoreGlobs], { cwd: root, absolute: true });
        const all = await (0, globby_1.default)(includeGlobs, { cwd: root, absolute: true });
        ...
        if (!actual.includes(filename) && all.includes(filename)) {
            for (const ignoreGlob of ignoreGlobs) {
                const withThisIgnore = await (0, globby_1.default)([...includeGlobs, ignoreGlob], { cwd: root, absolute: true });
