...
export class SwcCompiler {
    ...
    async initialize() {
        ...
        try {
            ...
            for (const file of files) {
                this.knownCacheEntries.add(file);
            }
        }
        catch (error) {
    ...
    async buildFile(filename, root, config) {
        ...
        if (this.knownCacheEntries.has(cacheKey)) {
            output = await fs.readFile(destination, "utf8");
        }
        else {
            const options = {
                cwd: root,
            ...
            const transformResult = await transform(content, options);
            output = transformResult.code;
