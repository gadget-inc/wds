...
const startFilesystemWatcher = (project) => {
    ...
    const reload = (path) => project.enqueueReload(path);
...
const startIPCServer = async (socketPath, project) => {
    const compile = async (filename) => {
        try {
            await project.compiler.compileOne(filename);
            project.watcher?.add(filename);
            return await project.compiler.fileDestinationGroup(filename);
        }
        catch (e) {
...
const esbuildDev = async (options) => {
    ...
    await project.rebuildAndReload();
    process.on("SIGINT", () => {
        project.shutdown(0);
    });
