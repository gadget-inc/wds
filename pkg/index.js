...
const startIPCServer = async (socketPath, project) => {
    const compile = async (filename) => {
        try {
            ...
            utils_1.log.error(`Error compiling file ${filename}, can't continue...`, e);
        }
    };
...
const esbuildDev = async (options) => {
    ...
    if (os_1.default.platform() === "win32") {
        syncSocketPath = path_1.default.join("\\\\?\\pipe", workDir, "ipc.sock");
    }
    else {
        syncSocketPath = path_1.default.join(workDir, "ipc.sock");
    }
    ...
    project.supervisor = new Supervisor_1.Supervisor([...childProcessArgs(), ...options.argv], syncSocketPath, options, project);
