...
class EsBuildCompiler {
    ...
    async destination(filename) {
        ...
        if (!result) {
            ...
            if (ignorePattern) {
                throw new Error(`File ${filename} is imported but not being built because it is explicitly ignored in the wds project config. It is being ignored by the provided glob pattern '${ignorePattern}', remove this pattern from the project config or don't import this file to fix.`);
            }
            else {
