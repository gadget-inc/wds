...
class Compiler {
    constructor(workspaceRoot, workDir) {
        ...
        this.fileToGroupMap = {};
        // a map from input filename to output filename
        this.fileToDestinationMap = {};
        this.destination = (filename) => {
            // make path relative to the workspace root
            const key = filename.slice(this.workspaceRoot.length + 1);
            return this.fileToDestinationMap[key];
        };
    }
    /**
        ...
        await this.reportESBuildErrors(async () => {
            ...
            for (const [output, details] of Object.entries(build.metafile.outputs)) {
                if (details.entryPoint) {
                    this.fileToDestinationMap[details.entryPoint] = path_1.default.resolve(output);
                }
            }
            return fileNames;
        });
